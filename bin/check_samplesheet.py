...
class RowChecker:
    ...
    def _validate_second(self, row):
        """Assert that the PBI entry has the right format if it exists."""
        if len(row[self._second_col]) > 0:
            self._validate_format(row[self._second_col])

    def _set_single_col_true(self, row):
